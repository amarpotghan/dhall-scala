...
object Expr extends ExprInstances with ExprFunctions {
...
private[dhall] sealed trait ExprFunctions {
  ...
  def substitute[S, T, A](variable: Var, which: Expr[S, A], in: Expr[T, A]): Expr[S, A] = {
    def subst[T]: Expr[T, A] => Expr[S, A] = substitute(variable, which, _)
    def withShift(label: String, expr: Expr[T, A]): Expr[S, A] = {
      val shifted = if(variable.label == label) variable.index + 1 else variable.index
      substitute(variable.copy(index = shifted), shiftVariableIndices(1, Var(label, 0), which), expr)
    }
